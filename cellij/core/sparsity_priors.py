...
def get_prior_function(
    ...
    def spikeandslabbeta_sample():
        return pyro.sample("w_scale", dist.Beta(torch.tensor(0.001), torch.tensor(0.001))).view(view_shape)

    def spikeandslab_continousbernoulli_sample():
        pi = pyro.sample("pi", dist.Beta(torch.tensor(0.001), torch.tensor(0.001))).view(view_shape)
        return pyro.sample("w_scale", dist.ContinuousBernoulli(probs=pi)).view(view_shape)

    def spikeandslab_relaxedbernoulli_sample():
        pi = pyro.sample("pi", dist.Beta(torch.tensor(0.001), torch.tensor(0.001))).view(view_shape)
        return pyro.sample(
            "w_scale", dist.RelaxedBernoulliStraightThrough(temperature=torch.tensor(0.1), probs=pi)
        ).view(view_shape)
    ...
    def horseshoe_sample():
        ...
        return torch.cat(
            [
                w_scale[..., feature_idx[view]] * feature_group_scale[..., idx : idx + 1]
    ...
    def lasso_sample():
        ...
        return pyro.sample("w_scale", dist.SoftLaplace(torch.tensor(0.0), torch.tensor(1.0))).view(view_shape)

    def nonnegative_sample():
        return pyro.sample("w_scale", dist.Normal(torch.tensor(0.0), torch.tensor(1.0))).view(view_shape)
    ...
    if not any([sparsity_prior == prior for prior in prior_functions]):
        ...
        raise ValueError(f"Sparsity prior '{sparsity_prior}' is not valid. Valid priors are {valid_priors}.")
