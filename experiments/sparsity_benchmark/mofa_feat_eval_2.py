...
for seed in [0, 1]:  # 2, 3, 4
    ...
    for grid_features in tqdm([400, 1000]):
        for N_FACTORS_ESTIMATED in [5, 10, 15, 20, 30, 40, 50, 60]:
            n_samples = N_SAMPLES
            ...
            for lr in [0.1, 0.01, 0.001]:  # , 0.0001
                for sparsity_prior, prior_params in [
                    ...
                    for idx, m in enumerate(mdata.mod):
                        ...
                        avg, all_k, _, _ = compute_factor_correlation(
                            w_true, w_hats[idx].T
                        )
                        perf_corr_factor[seed][grid_features][N_FACTORS_ESTIMATED][lr][
                            sparsity_prior
                        ] = avg
                        perf_corr_factor_all[seed][grid_features][N_FACTORS_ESTIMATED][
                            lr
                        ][
                            sparsity_prior
                    ...
                    # Timer
                    perf_time[seed][grid_features][N_FACTORS_ESTIMATED][lr][
                        sparsity_prior
                    ] = perf_timer
                    ...
                    for m, name in enumerate(mdata.mod):
                        for k in range(w_hats[m].shape[0]):
                            perf_w_activations_l1[seed][grid_features][
                                N_FACTORS_ESTIMATED
                            ][lr][sparsity_prior][m][k] = np.sum(np.abs(w_hats[m][k]))
                            perf_w_activations_l2[seed][grid_features][
                                N_FACTORS_ESTIMATED
                            ][lr][sparsity_prior][m][k] = np.sum(w_hats[m][k] ** 2)
                            x_hat_from_single_k = (
                                w_hats[m][k][:, None] @ z_hat[:, k][None, :]
                            )
                            x_hat_from_single_k = x_hat_from_single_k.T
                            perf_w_activations_ve[seed][grid_features][
                                N_FACTORS_ESTIMATED
                            ][lr][sparsity_prior][m][k] = r2_score(
                                x_true_splitted[m], x_hat_from_single_k
                            )
